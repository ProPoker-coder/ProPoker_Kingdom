...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO System_Settings (config_key, config_value) VALUES ('monthly_active', 'ON')") # 月榜預設開啟
...
if not st.session_state.player_id:
    st.markdown(f"""
        <div class="welcome-wall">
            ...
            <div class="feature-box"><b style="color:#FFD700; font-size:1.2em;">🎰 幸運獎項抽取</b><br>參與比賽累積 XP，物理抽取 iPhone、高級籌碼及實體精美物資。</div>
            <div class="feature-box"><b style="color:#FFD700; font-size:1.2em;">🛡️ 黑金物理核銷</b><br>物資由指揮部精確辨識序號有效性，保障每一位領主的獲獎權益。</div>
    ...
    for idx, row in gun_df.iterrows():
        with cols[idx % 3]:
            st.markdown(f'''<div style="background:#111; border:1px solid #444; border-radius:15px; padding:10px; text-align:center;">
                <img src="{row['img_url']}" style="width:100%; border-radius:10px; height:130px; object-fit:cover;">
                <p style="color:#FFD700; font-weight:bold; margin-top:10px;">{row['item_name']}</p>
                <p style="color:#FFF;">價值: {row['item_value']:,} XP</p>
                <p style="color:#666; font-size:0.8em;">庫存: {row['stock']}</p>
            </div>''', unsafe_allow_html=True)

with t_p[3]: # --- 【修復核心】：排行榜物理焊接與活動判定 ---
...
if st.session_state.access_level in ["老闆", "店長"]:
    ...
    with mt[1]: # 物資管理
        with st.form("ni"):
            ...
            img_f = st.file_uploader("圖片")
            if st.form_submit_button("🔨 執行物理上架"):
                img_d = f"data:image/png;base64,{base64.b64encode(img_f.read()).decode()}" if img_f else ""
                conn.execute("INSERT OR REPLACE INTO Inventory VALUES (?,?,?,?,?,?)", (nn, ns, nv, nw, img_d, n_mx)); conn.commit(); st.rerun()
        ...
        for _, ri in mdf.iterrows():
            with st.expander(f"📦 管理：{ri['item_name']}"):
                ...
                if st.button("💾 更新", key=f"u_{ri['item_name']}"): conn.execute("UPDATE Inventory SET stock=stock+?, weight=? WHERE item_name=?", (eq, ew, ri['item_name'])); conn.commit(); st.rerun()
    ...
    with mt[3]: # 📢 視覺與活動開關
        ...
        with col_act1:
            if st.button("🔓 開啟本月巔峰榜挑戰"):
                conn.execute("INSERT OR REPLACE INTO System_Settings VALUES ('monthly_active', 'ON')"); conn.commit(); st.rerun()
        with col_act2:
            if st.button("🔒 關閉本月巔峰榜挑戰"):
                conn.execute("INSERT OR REPLACE INTO System_Settings VALUES ('monthly_active', 'OFF')"); conn.commit(); st.rerun()
        
        if st.button("💾 物理保存視覺與碼設定"):
    ...
    with mt[4]: # 🎯 任命
        rid_v = st.text_input("調動 ID"); rl_v = st.selectbox("任命物理職位", ["玩家", "員工", "店長", "老闆"])
        if st.button("🪄 任命鋼印"):
    ...
    with mt[5]: # 🗑️ 結算
        if st.button("⚖️ 英雄規費削減"): conn.execute("UPDATE Leaderboard SET hero_points = MAX(0, hero_points - 150)"); conn.commit(); st.success("削減完成")
        if st.button("🔥 物理粉碎月榜"): conn.execute("DELETE FROM Monthly_God"); conn.commit(); st.rerun()
        if st.button("💀 物理粉碎總榜"): conn.execute("DELETE FROM Leaderboard WHERE player_id != '330999'"); conn.commit(); st.rerun()

    with mt[6]: # 📜 核銷辨識中心
        st.subheader("🎯 獎項序號核銷辨識中心")
        sid_v = st.number_input("輸入物理序號 ID", value=0, step=1)
        if st.button("🔥 物理核銷銷帳", type="primary"):
            p_chk = conn.execute("SELECT player_id, prize_name, status FROM Prizes WHERE id=?", (sid_v,)).fetchone()
            if not p_chk: st.error("❌ 查無序號！")
            elif p_chk[2] == '已核銷': st.warning("⚠️ 此序號已核銷過！")
            elif p_chk[2] == '待兌換':
                ...
                conn.commit(); st.success(f"✅ 核銷完成：{p_chk[1]} (玩家:{p_chk[0]})"); st.rerun()
        ...
        for _, rv in ldf_v.iterrows():
            ...
            with c_b:
                if st.session_state.access_level == "老闆" and st.button("🗑️", key=f"l_d_{rv['id']}"):
    ...
    with mt[7]: # 💾 備份
        ...
        rf = st.file_uploader("還原數據", type="db")
